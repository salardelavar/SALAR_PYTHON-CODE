...
# ############################################################################
"""
(1) The code uses a Newton‑Raphson scheme to find the two wall‑thickness parameters (Tx, Ty) that eliminate plan eccentricity in a single‑story building.
(2) An analysis function calculates stiffness centres (CR) and mass centres (CM) for a given Tx/Ty, using 12 columns, 4 shear walls (with fixed thickness offsets) and 5 floor masses.
(3) The eccentricities ex = x_CR – x_CM and ey = y_CR – y_CM are treated as error functions Fx, Fy to be driven to zero.
(4) Central finite differences (step 0.001 mm) approximate the derivatives ∂Fx/∂Tx and ∂Fy/∂Ty, assuming negligible coupling.
(5) Updates Tx,new = Tx – Fx/∂Fx and Ty,new = Ty – Fy/∂Ty are applied iteratively.
(6) Convergence is reached when the maximum absolute correction |ΔTx|, |ΔTy| falls below 10⁻⁶ mm.
(7) The initial guess is Tx=100 mm, Ty=1000 mm, and the loop runs up to 100 000 iterations if needed.
(8) Once converged, the optimum thicknesses of all four walls are reported, and the final CR–CM coincidence is verified.
(9) Total analysis time is recorded, and a plan plot confirms the aligned centres.
(10) In essence, the script automates the removal of horizontal irregularity by tuning two wall thicknesses until stiffness and mass centres coincide.
"""
def PLAN_IRREGULARITY_MASS_STIFFNESS_ANALYSIS_WITH_FLOOR_MASS_FUN(Tx, Ty, storey_height=3000.0):
    ...
    W1_T = Tx          # WALL 01 THICKNESS
    W2_T = Tx - 50.0   # WALL 02 THICKNESS
    W3_T = Ty - 150.0  # WALL 03 THICKNESS
    W4_T = Ty - 100.0  # WALL 04 THICKNESS  
    # 4 shear walls: [x_cent, y_cent, length, thickness, E, mass, orientation]
    shear_walls = np.array([
        [3000, 8000, 6000, W1_T, 28000, 2000, 0],         # W1: along x, resists y
        [9000, 0.0, 6000, W2_T, 28000, 2000, 0],          # W2: along x, resists y
        [0.0, 6000, 4000.0, W3_T, 28000, 2000, 1],        # W3: along y, resists x
        [12000.0, 2000.0, 4000.0, W4_T, 28000, 2000, 1],  # W4: along y, resists x
    ])
...
DEMANDx = 0.0     # Target Value is Difference between Center of Stiffness and Center of Mass
DEMANDy = 0.0     # Target Value is Difference between Center of Stiffness and Center of Mass

# Analysis Durations:
starttime = TI.process_time()

# ---------------------------------------------------------------------------
# FIND THE OPTIMUM VALUE (NEWTON-RAPHSON SOLVER FOR OPTIMAL X AND Y)
# ---------------------------------------------------------------------------
while (RESIDUAL > TOLERANCE):
    # X -------------------
    DATA = PLAN_IRREGULARITY_MASS_STIFFNESS_ANALYSIS_WITH_FLOOR_MASS_FUN(X, Y, storey_height=3000.0)
    ...
    Ymin = Y - ESP
    print('Xmin:    ', Xmin)
    DATA = PLAN_IRREGULARITY_MASS_STIFFNESS_ANALYSIS_WITH_FLOOR_MASS_FUN(Xmin, Ymin, storey_height=3000.0)
    x_CRmin, y_CRmin, x_CMmin, y_CMmin, Kx_tot, Ky_tot, K_tors, M_tot = DATA
    SUPPLYminX = x_CRmin - x_CMmin
    SUPPLYminY = y_CRmin - y_CMmin
    FminX = SUPPLYminX - DEMANDx
    FminY = SUPPLYminY - DEMANDx
    print('Fmin: ', FminX, FminY)
    # XMAX -------------------
    # Evaluate at Xmax and Fmax
    Xmax = X + ESP
    Ymax = Y + ESP
    print('Xmax:    ', Xmax)
    DATA = PLAN_IRREGULARITY_MASS_STIFFNESS_ANALYSIS_WITH_FLOOR_MASS_FUN(Xmax, Ymax, storey_height=3000.0)
    x_CRmax, y_CRmax, x_CMmax, y_CMmax, Kx_tot, Ky_tot, K_tors, M_tot = DATA
    SUPPLYmaxX = x_CRmax - x_CMmax
    SUPPLYmaxY = y_CRmax - y_CMmax
    FmaxX = SUPPLYmaxX - DEMANDx
    FmaxY = SUPPLYmaxY - DEMANDy
    print('Fmax: ', FmaxX, FmaxY)
    # DF -------------------
    DFx = (FmaxX - FminX)/(2 * ESP);# Calculate the Finite difference derivative of Fx
    DFy = (FmaxY - FminY)/(2 * ESP);# Calculate the Finite difference derivative of Fy

    print('DF:   ', DFx, DFy)
    # DX -------------------
    DXx = Fx / DFx; # Calculate dx
    DXy = Fy / DFy; # Calculate dy

    print('DX:   ', DXx, DXy)
    # RESIDUAL -------------------
    RESIDUAL = np.abs(max(DXx, DXy)); # Calculate residual
    print('IT: ', IT + 1, ' - RESIDUAL: ', RESIDUAL,' - X: ', X,' - Y: ', Y,'\n')
    X -= DXx; # update X
    Y -= DXy; # update Y
    ...
    if RESIDUAL < TOLERANCE:
        ...
        print(f'\t\t Optimum Wall 03 Thickness:       {Y-150.0:.4f}')
        print(f'\t\t Optimum Wall 04 Thickness:       {Y-100.0:.4f}')
