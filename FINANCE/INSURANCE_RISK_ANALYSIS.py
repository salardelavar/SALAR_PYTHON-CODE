...
# Sample data: Each row represents a driver with different features
data = {
    'Age': [25, 30, 22, 40, 55],
    'Car Value': [20000, 25000, 15000, 30000, 35000],
    'Previous Accidents': [1, 0, 3, 0, 0],
    'Driving Violations': [0, 1, 2, 0, 0],
    'Insurance Premium': [1200, 1100, 1500, 1000, 900]  # Example premiums based on the above factors
}
...
# Now, let's calculate the correlation between the factors and the premium
correlation_matrix = df.corr()

print("\nCorrelation between factors and insurance premium:")
print(correlation_matrix)

# Now let's do a simple linear regression to predict insurance premium based on the factors
from sklearn.linear_model import LinearRegression

# Features (independent variables)
X = df[['Age', 'Car Value', 'Previous Accidents', 'Driving Violations']]
...
# Let's predict the premium for a new driver with specific features
new_driver = pd.DataFrame({
    'Age': [28],
    'Car Value': [22000],
    'Previous Accidents': [1],
    'Driving Violations': [0]
})

predicted_premium = model.predict(new_driver)

print("\nPredicted insurance premium for the new driver:", predicted_premium[0])
