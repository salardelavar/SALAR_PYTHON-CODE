def CONCRETE_ELASTIC_SECTION_ANALYSIS_FUN():
    ...
    print(f"Section Area:            {A_total:.2f}")
    ...
    ax.plot(x_c, y_c, 'r+', label='Centroid')
    ax.set_aspect('equal')
    ax.set_title('Concrete Section')
    ax.legend()
    plt.xlabel('x')
    plt.ylabel('y')
    plt.grid(True)
...
#CONCRETE_ELASTIC_SECTION_ANALYSIS_FUN()    